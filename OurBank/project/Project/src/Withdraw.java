			...
			if (e.getSource()== withdrawButton) {
				...
				if (	CopiedAccount.isNumeric(Withdrawamount)){
					...
					if (MyFrame.CurrentAccount.inDebt) 
						...
						System.out.println(MyFrame.CurrentAccount);
					}
				}
				else{
					JOptionPane.showMessageDialog(null, "Invalid Input",
							"Hey!", JOptionPane.ERROR_MESSAGE);
					System.out.println("InValied account ! please try again");}
				}

			if (e.getSource()==goback) {
				frame.dispose();
				buttons create = new buttons();
				System.out.println(MyFrame.CurrentAccount.getBalance());
			}
		}
	 }