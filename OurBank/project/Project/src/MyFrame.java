		...
		public void actionPerformed(ActionEvent e) { 	
			if (e.getSource()==login) { 
				...
				if (isNumeric(custmerId)){
					CopiedAccount theAccount =new CopiedAccount(MyFrame.getname(), 0,MyFrame.getPasscode()); 
					number = Integer.parseInt(custmerId);
					for (CopiedAccount copiedAccount : accounts){
						if(copiedAccount.customerName.equals(custmerName)&& copiedAccount.customerPasscode==number ) {
							...
							System.out.println("Your ID is: " +custmerId);
